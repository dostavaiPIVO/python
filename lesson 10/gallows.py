...
while life > 0 and counter != len(word_answer):
    ...
    # пока есть жизни и не все буквы угаданы
    letter = input("буква: ")
    letter_is_be = False # наличие буквы в слове
    for i in range(len(word_answer)): # пробегаемся по всему слову
        if letter == word_answer[i]: # если буква равна букву из слова
            if word_display[i] != "_": # если буква уже отгадана
                letter_is_be = True
            else:
